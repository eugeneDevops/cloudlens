...
Поведение, которое код реализует или тест фиксирует. Решение —
в ADR-0002.
...
1. **Закрытый период принимает начисления.** Тесты
   ...
   Причина — поздние корректировки CUR. Пороги на закрытом периоде
   продолжают поднимать алерты. Лимит закрытого периода заморожен (№ 5),
   расход не заморожен. → ADR-0002
2. **Знак суммы.** Отрицательная сумма зерна принимается, `Spent` может
   ...
   закрепляет: все пороги `Clear`. → ADR-0002
3. **Порог 100% перезаряжается на 95%.** Гистерезис одинаковый для всех
   ...
   падения до 95%. Намеренно. → ADR-0002
4. **Бюджет — один период.** Код выбрал модель «одно поле `Period`»;
   открыть следующий период нечем. → ADR-0002
5. **Изменение расхода не поднимает события.** `ApplyCharge` без
   пересечения порога молчит. `ChargeApplied` отложен до дня 4.
   → ADR-0002
6. **Порядок проверок в `ChangeLimit`.** Сначала проверяются знак и
   ...
   вернётся не `ClosedPeriodLimit`. → ADR-0002
